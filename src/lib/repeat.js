...
function nextMonth(date, from, isPrevious) {
  ...
  if (currentDate > dayOfMonth) {
    ...
    nextDate = moment(from)
      ...
      .date(dayOfMonth); // set to upcoming month
  } else {
    // yet to happen, set to upcoming date in current month
...
function nextYear(date, from, isPrevious) {
  ...
  if (currentMonth > month) {
    ...
    nextDate = moment(from)
      .year(year + direction)
      .month(month)
      .date(dayOfMonth); // set to upcoming year
...
function nextSpan(_date, _span, _from, _every) {
  switch (_every) {
    case EXACT:
      return nextExactSpan(_date, _span, _from);
    case DAY:
    case WEEKDAY:
      return nextDaySpan(_date, _span, _from);
    case WEEK:
      return nextWeekSpan(_date, _span, _from);
    case MONTH:
      return nextMonthSpan(_date, _span, _from);
    case YEAR:
      return nextYearSpan(_date, _span, _from);
    default:
      return _span;
  }
}
...
function nextWeekSpan(_date, _span, _from) {
  let nextDate;
  const length = Math.round(moment(_span).diff(moment(_date), DAY, true));
  const numberOfDaysFromDate =
    Math.round(moment(_from).diff(moment(_date), DAY, true)) + 1; // inclusive
