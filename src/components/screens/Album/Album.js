...
class Album extends React.Component {
  ...
  _onDelete = async () => {
    ...
    const filtered = images.filter(
      (image) => !this.state.selected.includes(image.key),
    );
  ...
  render() {
    const {isSubmitting, loaded, total, selected, isLoading} = this.state;
    const {stores, images, loading, onRefresh, error, isOwner} = this.props;
      ...
      <>
        ...
        {isOwner && (
          <Fab
            ...
            onPress={
              selected.length ? this._onDelete : this._openDocumentPicker
            }
          />
        )}
      </>
