...
class CalendarDialog extends React.Component {
  ...
  _renderFooter = () => (
    <Button onPress={this._authorize}>
      {I18n.get('BUTTON_addMyCalendar')}
    </Button>
  );
  ...
  render() {
    const {stores, visible} = this.props;
        ...
        <Dialog
          ...
          <Dialog.Content>
            <FlatList
              ...
              ListFooterComponent={
                allCalendars.length ? undefined : this._renderFooter
              }
            />
          </Dialog.Content>
          {Boolean(allCalendars.length) && (
            <Dialog.Actions>
              <Button danger onPress={this._dimiss}>
                {I18n.get('BUTTON_dismiss')}
              </Button>
              <Button onPress={this._import}>
                {I18n.get('BUTTON_import')}
              </Button>
            </Dialog.Actions>
          )}
        </Dialog>
