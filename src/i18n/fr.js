...
export default {
  ...
  COMMENTS_loadMore: (count) =>
    `Afficher ${count} autre${count > 1 ? 's' : ''} commentaire${
      count > 1 ? 's' : ''
    }`,
  ...
  DISCOVER_locationUsage:
    'Schdlr a besoin de votre emplacement pour trouver des événements à proximité.',
  DISCOVER_emptyListCaption: 'Des événements qui se passent autour de vous!',

  SEARCH_inputPlaceholder: (city) =>
    `Rechercher${city ? ' ' + city : ' Schdlr'}`,
  ...
  NOTIFICATIONS_emptyUpdatesListCaption:
    'Des mises à jour et annonces importantes apparaîtront ici',
  ...
  ALERT_shortUntil:
    "L'événement doit se produire au moins deux fois avant la date finale",
  ALERT_whatIsASchedule: "Qu'est-ce qu'un horaire?",
  ALERT_whatIsAScheduleA: (schdl) =>
    `Un horaire est un groupe d'événements, comme un horaire de cours. ${
      schdl ? `Les "${schdl.name}"` : ''
    } seront informés de cet événement.`,
  ALERT_whatIsAScheduleA2:
    "Un calendrier est un groupe d'événements. Ajoutez des événements ici et les personnes qui suivent ce calendrier seront informées de vos événements.",
  ALERT_privateSchedule: 'Horaire privé',
  ALERT_privateScheduleA:
    'Les événements créés après avoir défini le calendrier sur «Privé» ne sont pas visibles par tout le monde.',
  ALERT_publicScheduleA:
    'Les événements créés après avoir défini le calendrier sur «Public» sont visibles par tous.',
  ALERT_permissionLocationTitle: 'Autorisation de localisation',
  ALERT_permissionLocationMessage:
    "Schdlr a besoin d'accéder à votre emplacement pour une meilleure expérience.",
  ...
  SHARE_SCHEDULE_subject:
    'Vous avez été invité à suivre un programme sur Schdlr.',
  ...
  TOAST_locationError:
    "Impossible d'obtenir l'emplacement. Désactivez le mode avion.",
  ...
  TOAST_newNotifications: (count) =>
    `${count} nouvelle${count > 1 ? 's' : ''} notification${
      count > 1 ? 's' : ''
    }`,
  ...
  TOAST_fileTooLarge: (name) =>
    `"${name.slice(0, 20)}..." est plus grand que 8 Mo`,
  ...
  SCHEDULES_loadPastEvents: (count) =>
    `charger (${count}) événement${count > 1 ? 's' : ''} passé`,
  ...
  HELPER_TEXT_invalidDatesAndRecur:
    "La durée de l'événement doit être plus courte que la fréquence de répétition",
  HELPER_TEXT_description: 'Trop long',
  HELPER_TEXT_website: 'Texte invalide',
  HELPER_TEXT_location:
    'Votre position nous aide à améliorer votre expérience avec de meilleurs résultats de recherche.',
  HELPER_TEXT_invalidEndDate:
    "L'événement devrait se terminer dans une date future",
  ...
  ERROR_serverError: (message) =>
    'Erreur du serveur' + (message ? ': ' + message : ''),
  ...
  ERROR_failedToGetLocation:
    "Impossible d'accéder à votre position. Vérifiez votre réseau mobile et votre GPS",
  ...
  ERROR_failedToCreateEvent: (title) =>
    `Impossible de publier l'événement: ${title}.`,
  ERROR_failedToCreateSchedule: (name) =>
    `Échec de l'enregistrement de la planification: ${name}.`,
  ERROR_failedToCreateComment: (content) =>
    `Échec de la remise du commentaire: ${content}.`,
  ERROR_failedToSendFiles: (count) =>
    `Failed to send ${count} file${count > 1 ? 's' : ''}`,
  ...
  DIALOG_deleteEventWarning:
    'Annulez toujours les événements avant de les supprimer pour informer vos abonnés.',
  DIALOG_deleteSchedule: 'Supprimer cet horaire?',
  DIALOG_deleteScheduleWarning:
    "L'action supprimera tous les événements associés!",
  ...
  REQUEST_LOCATION_MESSAGE:
    "L'application Schdlr a besoin d'accéder à votre position pour que les événements soient plus faciles à trouver.",

  PICKER_location: 'Emplacement',
  SYNC_message:
    'Supprime tous les événements supprimés et expirés et corrige les événements et les horaires manquants.',
  ...
  SELECT_customType: 'Type personnalisé...',
  topics: [
